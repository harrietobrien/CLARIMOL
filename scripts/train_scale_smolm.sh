...
# Comparison: 1.7B vs 7-8B models.
#
# Submit: sbatch scripts/train_scale_smolm.sh
...
echo "=== Scale Sweep: SmolLM2-1.7B ==="
nvidia-smi
echo "Start: $(date)"

OUT_DIR="output/scale_sweep/smolm2-1.7b"
...
for SEED in 42 137 2024; do
    ...
    echo ""
    echo "=== SmolLM2-1.7B seed_${SEED} ($(date)) ==="
...
if [ ! -f "$ZERO_DIR/results.json" ]; then
    echo ""
    echo "=== SmolLM2-1.7B zero-shot ($(date)) ==="
    mkdir -p "$ZERO_DIR"
    python -m clarimol evaluate \
